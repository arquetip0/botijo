    ...
    def run(self):
        ...
        while not self.stop_event.is_set() and eyes_active:
            try:
                ...
                faces = [d for d in detections if d["class_id"] < len(imx500.network_intrinsics.labels) and 
                        imx500.network_intrinsics.labels[d["class_id"]] == "face"]
                ...
                if kit and eyes_active:
                    try:
                        ...
                        if iteration % 20 == 0:  # Mostrar progreso cada 20 iteraciones
                            print(f"[EYES {iteration}] 🎯 X:{int(x_center)} Y:{int(y_center)} → "
                                  f"LR:{int(final_lr)}° UD:{int(final_ud)}° (conf:{best['confidence']:.2f})")
                    except Exception as e:
